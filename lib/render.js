...
function FillGaps(desc)
{
  let d = desc.map(e => {
    return {name: e.name, startAddr : Number(e.startAddr), endAddr: Number(e.endAddr)};
  });

  d.sort(compare);
  let PreviousEnd = -1;
  d.forEach(element => {
    if(element.startAddr != PreviousEnd + 1) {
      desc.push({name:'Unallocated', startAddr : '0x'+(PreviousEnd+1).toString(16).toUpperCase(), endAddr :'0x'+ (element.startAddr-1).toString(16).toLocaleUpperCase(), type:1});
    }
    PreviousEnd = element.endAddr;
  });
  return desc;
}
